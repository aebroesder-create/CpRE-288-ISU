...
void button_init() {
  ...
  while ((SYSCTL_PRGPIO_R & 0x0000010) == 0) {};
...
uint8_t button_getButton() {
	...
	// INSERT CODE HERE!
            ...
            // Read the Data Register for Port E
            // We mask with 0x0F because we only care about pins 0-3
            uint32_t data = GPIO_PORTE_DATA_R & 0x0F;

            // Check - Rightmost
            // If bit 3 is 0, the button is pressed
            if ((data & 0x08) == 0) {
                return 4;
            }
